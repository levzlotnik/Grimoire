...
### ❌ Anti-Pattern 1: Using bare component in bodies
```prolog
% WRONG
some_predicate :-
    component(E, prop, V).  % Use please_verify(component(...))!
...
### ❌ Anti-Pattern 2: Runtime assertions in tests
```prolog
% WRONG
setup_test :-
...
### ❌ Anti-Pattern 3: Python objects in semantics.pl
```prolog
% WRONG - semantics.pl should never see Python objects
implementation(conjure(thing(X)), Result, (
    py_call(module:func(X), PyObj),  % Should be in python_bridge.pl!
    Result = ok(PyObj)
))
```

### ❌ Anti-Pattern 4: Unclear spell invocation
```python
...
### ❌ Anti-Pattern 5: Bypassing session hooks
```prolog
...
- **`tmp/`** - Temporary files for local use
...
% If `setup_fn, cleanup_fn` are predicates with some data
% untracked by Grimoire functionality, e.g. creating/deleting aux files in /tmp/.grimoire/
test(my_test, [setup(setup_fn), cleanup(cleanup_fn)]) :-
    user:magic_cast(conjure(domain(action(...))), Result),
    assertion(Result = ok(_)).

% If the setup and cleanup are about some Grimoire-tracked resources
% you can do the setup and cleanup manually inside the test:
test(my_test_with_some_CRUD_operations) :-
    % Setup
    user:magic_cast(conjure(domain(create(...))), ResultCreate),
    ResultCreate = ok(...),
    assertion(ResultCreate = ok(domain_create_structure(_))),

    % Update some existing value in domain
    ValueToUpdate = value_to_update,
    user:magic_cast(conjure(domain(update(ValueToUpdate))), ResultUpdate),
    ResultUpdate = ok(...),
    assertion(ResultUpdate = ok(domain_update_structure(_))),

    % Read back the updated value
    user:magic_cast(perceive(domain(read(...))), ResultRead),
    ResultRead = ok(...),
    assertion(ResultRead = ok(domain_read_structure(ValueUpdated))),

    % Test that update->read preserved the updated value (if applicable for domain)
    assertion(ValueToUpdate == ValueUpdated),

    % Cleanup
    user:magic_cast(conjure(domain(delete(...))), ResultDelete),
    ResultDelete = ok(...),
    assertion(ResultDelete = ok(domain_delete_structure(_))).
